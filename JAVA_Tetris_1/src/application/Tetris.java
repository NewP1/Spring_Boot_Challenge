...
public class Tetris extends Application {
				...
				Platform.runLater(new Runnable() {
					public void run() {
						...
						if (game) {
							...
							level.setText("Lines: " + Integer.toString(LineNo));
						}
					}
				});
			}
			...
			do {
				...
				for (Node node : newrects) {
					...
					for (Node nodes : pane.getChildren()) {
						if (nodes instanceof Rectangle)
							rects.add(nodes);
					}

					for (Node nodes : rects) {
						Rectangle k = (Rectangle) nodes;
						try {
							MESH[(int) k.getX() / SIZE][(int) k.getY() / SIZE] = 1;
	...
	public void MoveDown(Form form) {
		...
		if (form.a.getY() == YMAX - SIZE || form.b.getY() == YMAX - SIZE || form.c.getY() == YMAX - SIZE
			...
			// creating a new block and ading to the scene
	...
	private boolean cB(Rectangle rect, int x, int y) {
		...
		if (y >= 0)
			yb = rect.getY() + x * MOVE > 0;
		if (y < 0)
			yb = rect.getY() + x * MOVE < YMAX;
